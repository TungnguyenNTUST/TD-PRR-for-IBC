...
class DiscreteBO:
    ...
    # def suggest_next(self, top_k: int = 1) -> Tuple[np.ndarray, np.ndarray]:
    #     """
    #     Suggest the next best candidate(s) by UCB.
    #     Returns (indices_1based, acq), both length top_k.
    #     """
    #     if self.ei_acq:
    #         if self._gp is None:
    #             raise RuntimeError("Model not fitted. Call initialize() first.")
    #
    #         # mask out observed points
    ...
    #         acq = self._ei(mu, std, y_best)
    #
    #         # pick top_k among unobserved, then map back to 1-based global indices
    #         unobs_idx = np.where(mask)[0]
    #         order = np.argsort(-acq)[:top_k]
    #         chosen_unobs = unobs_idx[order]
    #         return chosen_unobs + 1, acq[order]
...
#         allow_hyperparam_optimization: bool = False,
#         length_scale_bounds: Tuple[float, float] = (1e-5, 1e5),  # used only if optimization=True
#         sigma_f_bounds: Tuple[float, float] = (1e-5, 1e5),
#         sigma_n_bounds: Tuple[float, float] = (1e-10, 1e3),
#     ):
#         assert Input.ndim == 2, "Input must be shape (D, N)"
#         self.Input = Input
#         self.Xc = Input.T.copy()                   # shape (N, 8) for sklearn
#         self.N, self.D = self.Xc.shape
#         self.xi = float(xi)
#         self.normalize_y = bool(normalize_y)
#         self.random_state = random_state
...
#         if self._X_obs is None or self._y_obs is None or len(self._y_obs) == 0:
#             raise RuntimeError("No observations to fit. Call initialize() or tell() first.")
#         kernel, optimizer = self._make_kernel()
#         self._gp = GaussianProcessRegressor(
#             kernel=kernel,
#             optimizer=optimizer,
#             normalize_y=self.normalize_y,
...
#     def suggest_next(self, top_k: int = 1) -> Tuple[np.ndarray, np.ndarray]:
#         """
#         Suggest the next best candidate(s) by EI.
#         Returns (indices_1based, eis), both length top_k.
#         """
#         if self._gp is None:
#             raise RuntimeError("Model not fitted. Call initialize() first.")
#
...
#         # pick top_k among unobserved, then map back to 1-based global indices
#         unobs_idx = np.where(mask)[0]
#         order = np.argsort(-ei)[:top_k]
#         chosen_unobs = unobs_idx[order]
#         return chosen_unobs + 1, ei[order]
#
#     def rank_candidates(self, top_k: Optional[int] = None) -> pd.DataFrame:
#         """
#         Rank ALL unobserved candidates by EI (descending).
#         Returns a DataFrame with columns: index_1based, EI, and the 8 features.
#         """
#         if self._gp is None:
#             raise RuntimeError("Model not fitted. Call initialize() first.")
#
...
#             order = order[:top_k]
#
#         idx1 = unobs_idx[order] + 1
#         out = pd.DataFrame({
#             "index_1based": idx1,
#             "EI": ei[order],
#         })
#         # attach features x1..x8 for convenience
#         feats = self.Xc[unobs_idx[order], :]
#         for d in range(self.D):
#             out[f"x{d+1}"] = feats[:, d]
#         return out.reset_index(drop=True)
...
#         history = []
#         for it in range(1, n_iters + 1):
#             idxs, eis = self.suggest_next(top_k=1)
#             idx = int(idxs[0])
#             x = self.Xc[idx - 1, :]
#             y = float(evaluate_fn(idx, x))
#             self.tell(idx, y)
#             history.append({"iter": it, "index_1based": idx, "y": y, "EI": float(eis[0])})
#             if verbose:
#                 print(f"[BO] iter {it:02d}: picked {idx}, EI={eis[0]:.6g}, y={y:.6g}")
#         return pd.DataFrame(history)
#
#     def predict_all(self):
#         """Posterior mean & std for all N candidates (order = Input columns)."""
